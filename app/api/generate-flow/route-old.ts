...
import { configService, AI_CONFIG, COMMON_EXTERNAL_SERVICES, APP_CONFIG } from '@/lib/config';
...
function validateNodeContent(node: any) {
  ...
  const minContentLength = validationRules.MIN_CONTENT_LENGTH;
  const externalMinContentLength = validationRules.MIN_EXTERNAL_SERVICE_CONTENT_LENGTH;
  ...
  if (node.type === 'external') {
    // Additional services that are not in COMMON_EXTERNAL_SERVICES
...
export async function POST(request: Request) {
  try {
    ...
    const response = await client.chat.completions.create({
      ...
      temperature: configService.getGenerationParams().temperature,
      max_tokens: configService.getGenerationParams().maxTokens
    });
    ...
    if (!contentStructureValidation.isValid) {
      ...
      if (contentStructureValidation.errors.some(err => err.includes('语言') || err.includes('language'))) {
        ...
        // 声明 retryFlowData 用于存储重新生成结果，初始化为原始 flowData
        let retryFlowData: GeneratedFlow = flowData;
