...
- type：节点类型，取值必须为 "product"（产品功能/页面）、"external"（外部资源/服务）、"context"（上下文信息）或 "document"（文档）。
- title：节点标题，简短且唯一的描述。
- content：节点内容，必须是一个详细描述，用于指导开发者实现该页面或功能，内容必须遵循下面的具体要求。
- position：节点位置，必须是一个包含 x 和 y 数值的对象。

【特殊节点：文档节点】
- **必须**包含一个且只有一个类型为 "document" 的节点。
- 这个节点的 \`title\` 应该是整个项目的概括性标题。
- 这个节点的 \`content\` 必须是用户原始需求的完整、详细的重述和扩展，作为代码生成的主要依据。
